...
func legacy(r fiber.Router) {
	r.Get("/", func(c fiber.Ctx) error {
		...
		return sc.DoWithRetry(misc.HlsStreamingOnlyClient, req, c.Response())
	})
	...
	r.Get("/aac", func(c fiber.Ctx) error {
		...
		req.SetURI(parsed)
		req.Header.SetUserAgent(cfg.UserAgent)
		return sc.DoWithRetry(hls_aac_streaming_httpc, req, c.Response())
	})
		...
		for l := range bytes.SplitSeq(resp.Body(), newline) {
			...
			if l[0] == '#' {
				...
				if len(l) > len(x) && string(l[:len(x)]) == x {
					...
					c.Response().AppendBody(fasthttp.AppendQuotedArg(nil, l[len(x):len(l)-1]))
